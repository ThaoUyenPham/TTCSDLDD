            ...
            <div class="row">
                <div class="col-md-12">
                    <div class="list-ctkt text-right">
                        <a href="#" class="btn btn-info btn-sm" id="updateCategory">
                            <i class="ti-plus pdd-right-5"></i>
                            <span>Cập nhật</span>
                        </a>
                        <a href="#" class="btn btn-danger btn-sm" id="deleteCategory">
                            <i class="ti-plus pdd-right-5"></i>
                            <span>Xóa</span>
                        </a>
...
<script>
    $("#deleteCategory").click(function(){
        ...
        let editUrl = '{{ route("delete.category", ":id") }}';
            editUrl = editUrl.replace(':id', categoryID);
        if(confirm('bạn có chắc muốn xóa danh mục này, nếu xóa danh mục này có thể danh mục con và các bản đồ thuộc danh mục này sẽ xóa!!'))
        {
            $.ajax({
                type: 'PUT',
                url: editUrl,
    ...
    $('#updateCategory').click(function(){
        ...
        let categoryID = "{{ $inforcategory->id }}";
        let editUrl = '{{ route("update.category", ":id") }}';
            editUrl = editUrl.replace(':id', categoryID);
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        if(response.success=1)
                        {
                            ...
                            Swal.fire(
                                'Thông báo',
                                'Đã có lỗi sảy ra vui lòng kiểm tra lại',
                                'error' // các loại khác có thể là 'error', 'warning', 'info', 'question'
                            );
                            console.log(response);
                        }
